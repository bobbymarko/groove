...
- R33. Auto-pause (default on, Settings → Ride): while the workout runs, three seconds of zero watts pause it, and the first watts after that resume it; Start with a trainer reading zero waits at 0:00 for the pedals. A trainer silent for five seconds counts as zero. A manual Pause stays paused whatever the pedals do. Mirrors Zwift, where the workout stops when the rider stops. The coach shows "Start pedalling when you're ready" / "Paused. Pedal to carry on." (2026-09-08, from Bob's first week of riding.)
