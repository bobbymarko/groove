...
- R35. Pixel stability: the low-res frame is drawn at a whole number of screen pixels per texel (factor chosen from the Render height slider), the camera's yaw, pitch and sideways/vertical position are snapped to the texel grid before rendering, and the frame is shifted on screen by the sub-texel remainder (2-texel margin). MSAA 4× on the low-res viewport and a soft shadow edge stop thin branches and shadow bands flickering. Adapted from the 2D pixel-perfect smooth camera technique Bob researched. (2026-09-08.)
