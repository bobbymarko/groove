...
- R28. Apocalypse scene: ash ground, dead wood, rust sky. A horde spawns ahead at a rate that follows the target effort (nothing below 60 % FTP, about one every two seconds at threshold, nearly two a second at VO2), shambles toward the rider (running on hard blocks), and on contact is flung with a ballistic tumble scaled by speed and power, bounces on the terrain and dissolves. The rider kicks on each hit; kills show next to the reps counter. Model: "Animated Zombie" by Quaternius, CC BY 3.0 (attribution in assets/zombie/ATTRIBUTION.txt and the README). Later: interval-completion power-ups and weapons (Bob, 2026-09-04).
